...
def generate_datetime_list_mackSun(target_full_table: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """
    Generate a list of datetime objects and corresponding datetime strings
    from a FITS table containing separate date and time columns.
    ...
    ----------
    target_full_table : np.ndarray
        Numpy array representing a FITS table
    ...
    tuple of np.ndarray
        ...
        - datetime_str: np.ndarray of strings in "YYYY-MM-DD HH:MM:SS" format.
    """
    datetime_str = np.array(target_full_table)[:, 0]

    datetime_list = np.array([
        datetime.strptime(dt.split()[0], "%Y-%m-%dT%H:%M:%S.%f")
        for dt in datetime_str
    ])
...
def list_of_dicts_to_df(data: list[dict]) -> list[pd.DataFrame]:
    """
    Function responsible for converting a list of dictionaries, where each dictionary
    contains 'x' and 'y' keys, into a list of pandas DataFrames. The 'x' values are
    automatically converted from string to datetime.
    ...
    data : list of dict
        List of dictionaries with the structure::
            ...
            [
                {
                    "x": list of str or datetime
                        List of datetime values (as strings or datetime objects) for the event.
                    "y": list of float
                        List of corresponding numeric values for the event.
                },
                ...
            ]
    ...
    list of pandas.DataFrame
        A list of DataFrames, where each DataFrame corresponds to one dictionary from
        the input list and contains two columns: 'x' (as datetime) and 'y'.
    """
    return [pd.DataFrame(d).assign(x=pd.to_datetime(d["x"])) for d in data]

def get_csv_data_to_download(df_list: list[pd.DataFrame]) -> bytes:
    """
    Create a ZIP file in memory containing CSVs for each DataFrame in df_list.
    Each CSV file is named using the first and last datetime from column 'x'.

    Parameters
    ----------
    df_list : list of pandas.DataFrame
        List of DataFrames, each containing at least a column 'x' of datetimes.

    Returns
    -------
    bytes
        The binary content of the generated ZIP file.
    """
    zip_buffer = BytesIO()
    with zipfile.ZipFile(zip_buffer, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
        for df in df_list:
            start = df["x"].iloc[0].strftime("%Y%m%d_%H%M%S")
            end = df["x"].iloc[-1].strftime("%Y%m%d_%H%M%S")
